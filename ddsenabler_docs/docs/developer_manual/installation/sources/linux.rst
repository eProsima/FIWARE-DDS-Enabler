...
The instructions for installing the |ddsenabler| application from sources and its required
...
|ddsenabler| depends on *eProsima Fast DDS* library and certain Debian packages.
This section describes the instructions for installing |ddsenabler| dependencies and requirements in a Linux
...
The installation of |ddsenabler| in a Linux environment from sources requires the following tools to be
...
These packages provide the tools required to install |ddsenabler| and its dependencies from command line.
...
By default, |ddsenabler| does not compile tests.
...
It is also possible to clone the Gtest_ Github repository into the |ddsenabler| workspace and compile it with colcon_
...
|ddsenabler| has the following dependencies, when installed from sources in a Linux environment:
...
yaml-cpp is a YAML parser and emitter in C++ matching the YAML 1.2 spec, and is used by *DDS Enabler* application to
...
this library when building the |ddsenabler| application by using the command:

.. code-block:: bash

    source <fastdds-installation-path>/install/setup.bash

In other case, just download *Fast DDS* project from sources and build it together with |ddsenabler| using colcon
...
#.  Create a :code:`DDS-Enabler` directory and download the :code:`.repos` file that will be used to install
    |ddsenabler| and its dependencies:

    .. code-block:: bash

        mkdir -p ~/DDS-Enabler/src
        cd ~/DDS-Enabler
        wget https://raw.githubusercontent.com/eProsima/FIWARE-DDS-Enabler/main/ddsenabler.repos
    ...
    .. note::
        ...
        It is just needed to download and build the |ddsenabler| project having sourced its dependencies.
...
This section explains how to compile |ddsenabler| with CMake_, either
...
#.  Create a :code:`DDS-Enabler` directory where to download and build |ddsenabler| and its dependencies:
...
#.  Once all dependencies are installed, install |ddsenabler|:

    .. code-block:: bash

        # ddsenabler_core
        cd ~/DDS-Enabler
        mkdir build/ddsenabler_core
        cd build/ddsenabler_core
        cmake ~/DDS-Enabler/src/ddsenabler/ddsenabler_core -DCMAKE_INSTALL_PREFIX=~/DDS-Enabler/install -DCMAKE_PREFIX_PATH=~/DDS-Enabler/install
        cmake --build . --target install
        ...
        cmake ~/DDS-Enabler/src/ddsenabler/ddsenabler_yaml -DCMAKE_INSTALL_PREFIX=~/DDS-Enabler/install -DCMAKE_PREFIX_PATH=~/DDS-Enabler/install
        cmake --build . --target install
        ...
        cmake ~/DDS-Enabler/src/ddsenabler/tools/ddsenabler_tool -DCMAKE_INSTALL_PREFIX=~/DDS-Enabler/install -DCMAKE_PREFIX_PATH=~/DDS-Enabler/install
        cmake --build . --target install
...
To install |ddsenabler| system-wide instead of locally, remove all the flags that
appear in the configuration steps of :code:`Fast-CDR`, :code:`Fast-DDS`, and
:code:`DDS-Enabler`, and change the first in the configuration step of :code:`foonathan_memory_vendor` to the
following:
...
To run the |ddsenabler| application, TODO.
